...
class SettingController extends Controller
{
    ...
    public function update(Request $request, string $group): RedirectResponse
    {
        ...
        // Ensure for AI settings that only one provider is marked active
        if ($group === 'ai') {
            $activeProvider = $data['active_ai_provider'] ?? 'openai';
            $providers = ['openai', 'anthropic', 'gemini', 'deepseek', 'groq', 'custom'];
            foreach ($providers as $p) {
                $data["{$p}_enabled"] = ($activeProvider === $p) ? '1' : '0';
            }
            $data['active_ai_provider'] = $activeProvider;
        }
