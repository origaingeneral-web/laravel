...
const DEMO_EMAIL = 'demo@kt.com';
const DEMO_PASSWORD = 'demo123';

export function shouldUseDemoAuth(email: string, password: string): boolean {
...
export const SupabaseAdapter = {
  ...
  async login(email: string, password: string): Promise<AuthModel> {
    ...
    if (shouldUseDemoAuth(email, password)) {
      console.log('SupabaseAdapter: Using built-in demo auth fallback');
      return {
